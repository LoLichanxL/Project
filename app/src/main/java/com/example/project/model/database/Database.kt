...
class Database {
    companion object {
        val database =
            Firebase.database("https://project-24d51-default-rtdb.europe-west1.firebasedatabase.app/")
        ...
        fun addChat(chat: Chat, firstUID: String, secondUID: String) {
            database.reference.child("Chats").child(chat.chatID).child("members")
                .setValue(listOf(firstUID, secondUID))
        }
        ...
        fun addChatMessage(chatID: String, message: Message) {
            database.reference.child("ChatMessages").child(chatID).child("messages").get()
                .addOnSuccessListener {
                    if (it.getValue() == null) {
                        database.reference.child("ChatMessages").child(chatID).child("messages")
                            .setValue(
                                listOf(message)
                            )
        ...
        fun uploadAdvertImage(advertID: String, count: Int, uri: String) {
            if (count == 1) {
                val uploadTask =
                    storage.reference.child("adverts/" + advertID + "/firstImage.jpg").putFile(
                        Uri.parse(uri)
                    )
            }
            if (count == 2) {
                val uploadTask =
                    storage.reference.child("adverts/" + advertID + "/secondImage.jpg").putFile(
                        Uri.parse(uri)
                    )
            }
            if (count == 3) {
                val uploadTask =
                    storage.reference.child("adverts/" + advertID + "/thirdImage.jpg").putFile(
                        Uri.parse(uri)
                    )
            }
        }
        ...
        fun uploadImageAdvertImageFromImageView(advertID: String, count: Int, data: ByteArray) {
            if (count == 1) {
                var uploadTask = storage.reference.child("adverts/" + advertID + "/firstImage.jpg")
                    .putBytes(data)
            }
            if (count == 2) {
                var uploadTask = storage.reference.child("adverts/" + advertID + "/secondImage.jpg")
                    .putBytes(data)
            }
            if (count == 3) {
                var uploadTask = storage.reference.child("adverts/" + advertID + "/thirdImage.jpg")
                    .putBytes(data)
            }
        }
